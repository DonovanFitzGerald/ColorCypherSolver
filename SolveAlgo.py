...
def Algo45(cypher, colorDict, colors, cypherLength):
    ...
    while turnCount < 10000:  # Start of 4
        ...
        for i in range(len(postitionDict)):
            ...
            if valueTotal != 0:
                for j in range(len(postitionDict[i])):
                    postitionDict[i][colors[j]] = (
                        postitionDict[i][colors[j]] / valueTotal
                    )
...
def Algo5Shannon(cypher, colorDict, colors, cypherLength):
    ...
    def getProbabiltyDict(allPossibilities):
    ...
    positionPossibilitiesDict = getProbabiltyDict(allPossibilities)

    turnCount = 1
    while turnCount < 10000:
        if turnCount == 1:
            guess = []
            guess.append(colors[1] * (cypherLength / 2))
            guess.append(colors[0] * (cypherLength / 2))
        else:
        # print(cypher, guess)
